...
class Utilities {
  ...
  class func openTab(from page: SFSafariPage, userInfo: [String : Any]?, makeActive: Bool) {
    ...
    page.getContainingTab { (tab) in
      tab.getContainingWindow(completionHandler: { (window) in
        window?.openTab(with: url, makeActiveIfPossible: makeActive, completionHandler: { (tab) in
          if tab != nil {
            NSLog("opened tab")
          }
        })
